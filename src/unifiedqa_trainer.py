...
class UnifiedQATrainer:
    ...
    def evaluate(self, epoch, dataset_name, dataset, logfile):
        ...
        results_df.to_excel(
            f'{self.run_files}/predictions_{dataset_name}_{"pretrained" if epoch == -1 else epoch}.xlsx', index=False)
    ...
    def evaluate_marriages(self, epoch, dataset_name, dataset, logfile):
        ...
        predictions_to_save = []
        for batch in tqdm(dataloader):
            encoding = self.tokenizer(batch['input'], padding="longest", max_length=max_source_length,
                                      truncation=True,
                                      return_tensors="pt")
            input_ids, attention_mask = encoding.input_ids, encoding.attention_mask
        ...
        results_df = pd.DataFrame(predictions_to_save,
                                  columns=['Input', 'Correct', 'Prediction', 'Is Correct prediction'])
        # results_df.to_excel(
        #     f'{self.run_files}/predictions_{dataset_name}_{"pretrained" if epoch == -1 else epoch}.xlsx',
        #     index=False)
        results_df.to_csv(
            f'{self.run_files}/predictions_{dataset_name}_{"pretrained" if epoch == -1 else epoch}.csv',
            index=False)
        if epoch == -1 or 'train' in dataset_name:
            self.best_score[dataset_name] = f1
            return
